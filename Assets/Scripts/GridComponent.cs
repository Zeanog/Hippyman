...
namespace Neo
{
    ...
    public class GridComponent : MonoBehaviour
    {
        ...
        {
            float hStart = transform.position.x - ((NumTiles.x - 1) * 0.5f) * TileDiameter;;
            float vStart = transform.position.z - ((NumTiles.y - 1) * 0.5f) * TileDiameter;
        ...
        {
            int hNumTiles = (int)(size.x / TileDiameter);
            int vNumTiles = (int)(size.z / TileDiameter);

            float hStart = pos.x - (((float)hNumTiles - 1f) * 0.5f) * TileDiameter;
            float vStart = pos.z - (((float)vNumTiles - 1f) * 0.5f) * TileDiameter;
            ...
            for (int iy = 0; iy < vNumTiles; ++iy)
            {
                for (int ix = 0; ix < hNumTiles; ++ix)
                {
                    try
                    {
                        WorldToGrid(new Vector3(hStart + ix * TileDiameter, 0f, vStart + iy * TileDiameter), out Vector2Int tile);
